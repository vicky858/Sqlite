...
-(BOOL) addDirectoryToZip:(NSString*) directory{
        
    
    NSError *error = nil;
    NSArray *array1 = [[NSFileManager defaultManager] subpathsOfDirectoryAtPath:directory error:&error];
    
    NSDictionary *attribs;
    
    for (id filName in array1) {
               
        attribs = [[NSFileManager defaultManager] attributesOfItemAtPath:[NSString stringWithFormat:@"%@/%@",directory,filName] error: NULL];
        
        if ([filName hasSuffix:@".DS_Store"]) {
            //            NSLog(@"Temp:%@",filName);            
        }else{
          if ([attribs objectForKey: NSFileType] == NSFileTypeRegular) {
              
              BOOL ret = [self addFileToZip:[directory stringByAppendingPathComponent:filName] newname:filName];
                if (ret == NO) {
                    NSLog(@"zip error");
                }
            }
...
-(BOOL) addFileToZipData:(NSData*) fileData newname:(NSString*) newname;
{
	if( !_zipFile )
		return NO;
    //	tm_zip filetime;
...
//	}
	...
	{
		data = fileData;// [ NSData dataWithContentsOfFile:file];
		uLong crcValue = crc32( 0L,NULL, 0L );
		crcValue = crc32( crcValue, (const Bytef*)[data bytes], [data length] );
		ret = zipOpenNewFileInZip3( _zipFile,
                                   (const char*) [newname UTF8String],
...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
//			NSLog([NSString stringWithFormat:@"%llu entries in the zip file",globalInfo.number_entry] );
		}
	}
